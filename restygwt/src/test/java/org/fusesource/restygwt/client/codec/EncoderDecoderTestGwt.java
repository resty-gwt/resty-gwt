...
public class EncoderDecoderTestGwt extends GWTTestCase {
	...
	public interface WrapperLibraryCodec extends
			JsonEncoderDecoder<LibraryWithWrapper> {
	}

	public interface ArrayWrapperLibraryCodec extends
			JsonEncoderDecoder<LibraryWithArrayWrapper> {
	}

	public interface PropertyLibraryCodec extends
			JsonEncoderDecoder<LibraryWithProperty> {
	}
	...
	public void testSubtypeArrayWrappeObjectWithSingleSubtype() {
		ArrayWrapperLibraryCodec lc = GWT
				.create(ArrayWrapperLibraryCodec.class);
	...
	public interface CreatorCodec extends
			JsonEncoderDecoder<CredentialsWithCreator> {
	}

	public void testCreators() {
		CreatorCodec codec = GWT.create(CreatorCodec.class);
		CredentialsWithCreator c = new CredentialsWithCreator("email",
				"password");
		c.age = 12;
	...
	public interface WrapperCodec extends
			JsonEncoderDecoder<CredentialsWithWrapperObject> {
	}

	public interface SubWrapperCodec extends
			JsonEncoderDecoder<SubCredentialsWithWrapperObject> {
	}
	...
	public void testSubtypeWrapperObject() {
		...
		assertFalse(baseRoundTrip.getClass().equals(
				SubCredentialsWithWrapperObject.class));
		...
		SubCredentialsWithWrapperObject subRoundTrip = (SubCredentialsWithWrapperObject) codec
				.decode(subJson);
	...
	public interface PropertyCodec extends
			JsonEncoderDecoder<CredentialsWithProperty> {
	}

	public interface SubPropertyCodec extends
			JsonEncoderDecoder<SubCredentialsWithProperty> {
	}
	...
	public void testSubtypeProperty() {
		...
		assertEquals("password-super", baseRoundTrip.password);
		assertFalse(baseRoundTrip.getClass().equals(
				SubCredentialsWithProperty.class));
		...
		sub.login = "login-sub";
		JSONValue subJson = codec.encode(sub);
		SubCredentialsWithProperty subRoundTrip = (SubCredentialsWithProperty) codec
				.decode(subJson);
		assertEquals("email-sub", subRoundTrip.email);
		assertEquals("password-sub", subRoundTrip.password);
		assertEquals("login-sub", subRoundTrip.login);
	...
	// AbstractJsonEncoderDecoder.STRING.decode(AbstractJsonEncoderDecoder.INT.encode(i)));
	// }
	//
