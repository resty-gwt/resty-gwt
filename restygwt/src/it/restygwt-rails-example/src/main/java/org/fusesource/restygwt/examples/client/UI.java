...
public class UI implements EntryPoint {
    ...
    static public interface PizzaRequest extends RequestContext {
...
//    	  InstanceRequest<PizzaProxy, Void> persist();

  //  	  InstanceRequest<PizzaProxy, Void> remove();

    	}

    static interface PizzaRequestFactory extends RequestFactory {
	...
	PizzaOrder.PizzaOrderJED codec = GWT.create(PizzaOrder.PizzaOrderJED.class);

	JSONValue request = codec.encode(order);

	resource.post().json(request).send(new JsonCallback() {
            public void onSuccess(Method method, JSONValue receipt) {
            	OrderConfirmation.OrderConfirmationJED codec = GWT.create(OrderConfirmation.OrderConfirmationJED.class);

                RootPanel.get().add(new Label("got receipt: " + codec.decode(receipt)));
            }
